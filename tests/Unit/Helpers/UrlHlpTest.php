...
class UrlHlpTest extends TestCase
{
    ...
    {
        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 0);
        config()->set('newt.hash_size_2', 0);
        $this->assertSame(0, UrlHlp::url_key_capacity());

        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 1);
        config()->set('newt.hash_size_2', 2);
        $this->assertSame(12, UrlHlp::url_key_capacity()); // (3^1)+(3^2)

        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 2);
        config()->set('newt.hash_size_2', 2);
    ...
    {
        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 2.7);
        config()->set('newt.hash_size_2', 3);
        $this->assertSame(36, UrlHlp::url_key_capacity()); // (3^2)+(3^3)

        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 2);
        config()->set('newt.hash_size_2', 3.7);
        $this->assertSame(36, UrlHlp::url_key_capacity()); // (3^2)+(3^3)
    }
    ...
    {
        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 'string');
        config()->set('newt.hash_size_2', 2);
        $this->assertSame(0, UrlHlp::url_key_capacity());

        config()->set('newt.hash_alphabet', 'abc');
        config()->set('newt.hash_size_1', 2);
        config()->set('newt.hash_size_2', 'string');
        // $alphabet_length^$hash_size_1 or 3^2
        $this->assertSame(9, UrlHlp::url_key_capacity());
