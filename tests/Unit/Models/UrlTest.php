...
class UrlTest extends TestCase
{
     ...
     */
    public function url_key_capacity($hash_size, $expected)
    {
        config()->set('urlhub.hash_size', $hash_size);
    ...
    public function urlKeyCapacityProvider()
    {
        return [
            [0, 0],
            [1, 3], // (3^1)
            [2, 9], // $alphabet_length^$hash_size or 3^2
    ...
    public function url_key_remaining()
    {
        ...
        config()->set('urlhub.hash_size', 1);

        // 3 - 5 = must be 0
        $this->assertSame(0, $this->url->url_key_remaining());

        config()->set('urlhub.hash_size', 2);
